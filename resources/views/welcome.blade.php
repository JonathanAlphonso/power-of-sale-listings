...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ config('app.name', 'Power of Sale Ontario') }}</title>
    ...
    <body class="flex min-h-screen flex-col overflow-x-hidden bg-[#f6f8fb] text-slate-800">
        <div class="pointer-events-none absolute inset-0">
            ...
            <header class="sticky top-0 z-50 border-b border-white/60 bg-white/95 shadow-[0_10px_30px_rgba(15,23,42,0.06)] backdrop-blur">
                <nav class="mx-auto flex max-w-6xl flex-wrap items-center gap-4 px-6 py-6 text-sm font-medium text-slate-600 lg:px-8">
                    <a href="#" class="flex flex-1 items-center gap-3 text-lg font-semibold tracking-tight text-slate-900">
                        Power of Sale Ontario
                        <span class="inline-flex items-center gap-1 rounded-full border border-red-200 bg-red-50 px-2 py-0.5 text-xs font-semibold uppercase tracking-[0.3em] text-red-600">
                            🇨🇦 Made in Canada
                        </span>
                    </a>
                    <div class="hidden flex-1 items-center justify-center gap-8 md:flex">
                        ...
                        <a href="#roadmap" class="transition hover:text-slate-900">Roadmap</a>
                        <a href="#pipeline" class="transition hover:text-slate-900">Pipeline</a>
                        <a href="#tech" class="transition hover:text-slate-900">Stack</a>
                    </div>
                    <div class="flex flex-1 items-center justify-end gap-3">
                        @if ($hasLogin)
                            <a href="{{ route('login') }}" class="rounded-full border border-slate-200 px-4 py-2 text-slate-600 transition hover:border-red-500/40 hover:text-red-500">Sign in</a>
                        @endif

                        @if ($hasRegister)
                            <a href="{{ route('register') }}" class="rounded-full bg-[#d90429] px-4 py-2 font-semibold text-white transition hover:bg-[#b90322]">Join waitlist</a>
                        @else
                            <a href="#cta" class="rounded-full bg-[#d90429] px-4 py-2 font-semibold text-white transition hover:bg-[#b90322]">Request access</a>
                        @endif
                    </div>
                </nav>
            </header>
                            ...
                            @if ($dbConnected)
                                <dl class="mt-8 grid gap-6 sm:grid-cols-3">
                                    ...
                                    <div class="rounded-2xl border border-slate-200 bg-slate-50/60 p-6">
                                        ...
                                        <p class="mt-2 text-sm text-slate-600">Reflects records in the <code class="whitespace-nowrap text-xs text-slate-500">users</code> table, when present.</p>
                                    </div>
                                </dl>
                            @else
                                <div class="mt-8 rounded-2xl border border-red-200 bg-red-50 p-6 text-sm text-red-700">
                                    <p>Unable to reach the database using the current configuration.</p>
                                    @if ($dbErrorMessage)
            ...
            <footer class="relative z-10 mt-auto border-t border-slate-200 bg-white/90 px-6 py-10 text-sm text-slate-600 lg:px-8">
                <div class="mx-auto flex max-w-6xl flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <p>&copy; {{ now()->year }} Power of Sale Ontario. Built with Laravel, Livewire, and Tailwind in Canada.</p>
                    <div class="flex flex-wrap items-center gap-4">
                        <a href="#roadmap" class="transition hover:text-[#d90429]">Roadmap</a>
                        <a href="#tech" class="transition hover:text-[#d90429]">Stack</a>
                        <a href="mailto:hello@powerofsale.ca" class="transition hover:text-[#d90429]">Contact</a>
                    </div>
                </div>
            </footer>
    </body>
