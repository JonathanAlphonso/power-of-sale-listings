...
## ⚠️ CRITICAL: Tests Must Use SQLite In-Memory Database

**Tests MUST run against SQLite in-memory database, NOT the production MySQL database.**

The `phpunit.xml` configures tests to use SQLite:
```xml
<env name="DB_CONNECTION" value="sqlite"/>
<env name="DB_DATABASE" value=":memory:"/>
```

If users are being dropped during tests, the test is likely bypassing this configuration.

**After running tests, ALWAYS reseed users:**
```bash
php artisan db:seed
```
...
- **board_code must NEVER be empty** for listing data
- The board ID is always available in one of these 3 API response fields:
  - `ListAOR`
  - `OriginatingSystemName`
  - `SourceSystemName`
- Example: `"ListAOR": "Toronto Regional Real Estate Board"`
