...
it('shows queued notice prominently and with at least 10s visibility', function (): void {
    ...
    // Assert the callout includes an Alpine timer for at least 10 seconds (we set 10000ms)
    expect($html)->toContain('x-init="setTimeout(() => visible = false, 10000)');
    expect($html)->toContain('data-min-visible-ms="10000"');

    // Assert the notice appears before the main status section so users see it immediately
    $noticePos = strpos($html, 'data-min-visible-ms="10000"');
