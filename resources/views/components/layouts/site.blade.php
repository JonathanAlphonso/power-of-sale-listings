    ...
    <body class="min-h-screen bg-slate-50 text-slate-900 antialiased dark:bg-zinc-950 dark:text-zinc-100">
        <div class="flex min-h-screen flex-col">
                ...
                @else
                    <div class="hidden items-center gap-2 lg:flex">
                        ...
                        @if (Route::has('register'))
                            <flux:button variant="primary" :href="route('register')" wire:navigate>{{ __('Create account') }}</flux:button>
                        @endif
                    </div>
                @endauth
            </flux:header>

            <flux:sidebar stashable sticky class="lg:hidden border-r border-slate-200 bg-white dark:border-zinc-800 dark:bg-zinc-900">
            ...
            <footer class="border-t border-slate-200 bg-white/90 dark:border-zinc-800 dark:bg-zinc-900/90">
                <div class="mx-auto flex max-w-6xl flex-col gap-6 px-6 py-10 sm:flex-row sm:items-center sm:justify-between lg:px-8">
                    <div class="flex items-center gap-3">
                        <x-app-logo-icon class="h-8 w-8 text-emerald-500 dark:text-emerald-400" />
                        <div>
                            <p class="text-sm font-semibold">{{ config('app.name') }}</p>
                            <p class="text-xs text-slate-500 dark:text-zinc-400">{{ __('Ontario foreclosure intelligence, powered by Laravel & Livewire.') }}</p>
                        </div>
                    </div>
                    <div class="flex items-center gap-4 text-sm text-slate-500 dark:text-zinc-400">
                        <a href="{{ route('home').'#roadmap' }}" class="hover:text-slate-900 dark:hover:text-zinc-200">{{ __('Roadmap') }}</a>
                        <a href="{{ route('home').'#cta' }}" class="hover:text-slate-900 dark:hover:text-zinc-200">{{ __('Contact') }}</a>
                        <a href="https://github.com/laravel/livewire-starter-kit" target="_blank" class="hover:text-slate-900 dark:hover:text-zinc-200">
                            {{ __('Repository') }}
                        </a>
                    </div>
