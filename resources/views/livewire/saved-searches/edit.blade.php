...
new #[Layout('components.layouts.site', ['title' => 'Edit Saved Search'])] class extends Component {
    ...
    public function mount(SavedSearch $savedSearch): void
    {
        Gate::authorize('update', $savedSearch);

        $this->savedSearch = $savedSearch;

        // Populate form from saved search
        $this->name = $savedSearch->name;
        $this->notification_channel = $savedSearch->notification_channel ?? 'email';
        $this->notification_frequency = $savedSearch->notification_frequency ?? 'daily';
        $this->is_active = $savedSearch->is_active;

        // Populate filters
        $filters = $savedSearch->filters ?? [];
        $this->search = $filters['q'] ?? '';
        $this->status = $filters['status'] ?? '';
        $this->municipalityId = (string) ($filters['municipality'] ?? '');
        $this->propertyType = $filters['type'] ?? '';
        $this->minPrice = (string) ($filters['min_price'] ?? '');
        $this->maxPrice = (string) ($filters['max_price'] ?? '');
        $this->minBedrooms = (string) ($filters['beds'] ?? '');
        $this->minBathrooms = (string) ($filters['baths'] ?? '');
    }

    public function save(): void
    {
        Gate::authorize('update', $this->savedSearch);
...
<section class="mx-auto max-w-2xl px-6 py-12 lg:px-8">
    <div class="space-y-2">
        ...
        <h1 class="text-3xl font-semibold tracking-tight text-slate-900 dark:text-zinc-100">
            {{ __('Edit Saved Search') }}
        </h1>
        <p class="text-sm text-slate-600 dark:text-zinc-400">
            {{ __('Update your search criteria and notification preferences.') }}
        </p>
    </div>
